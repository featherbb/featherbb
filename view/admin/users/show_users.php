...
<div class="linkst">
	<div class="inbox crumbsplus">
		<ul class="crumbs">
			<li><a href="<?php echo $feather->url->get_link('admin/') ?>"><?php _e('Admin').' '.__('Index') ?></a></li>
			<li><span>»&#160;</span><a href="<?php echo $feather->url->get_link('admin/users/') ?>"><?php _e('Users') ?></a></li>
    ...
    if ($info['num_posts']) {
        ...
        foreach ($info['posters'] as $cur_poster) {
            if (isset($info['user_data'][$cur_poster['poster_id']])) {
                ?>
				<tr>
					<td class="tcl"><?php echo '<a href="'.$feather->url->get_link('user/'.$info['user_data'][$cur_poster['poster_id']]['id'].'/').'">'.feather_escape($info['user_data'][$cur_poster['poster_id']]['username']).'</a>' ?></td>
					...
					<td class="tcr"><?php echo '<a href="'.$feather->url->get_link('admin/users/ip-stats/id/'.$info['user_data'][$cur_poster['poster_id']]['id'].'/').'">'.__('Results view IP link').'</a> | <a href="search.php?action=show_user_posts&amp;user_id='.$info['user_data'][$cur_poster['poster_id']]['id'].'">'.__('Results show posts link').'</a>' ?></td>
				</tr>
<?php
