    ...
    if ($info['num_posts']) {
        ...
        foreach ($info['posters'] as $cur_poster) {
            if (isset($info['user_data'][$cur_poster['poster_id']])) {
                ?>
				<tr>
					<td class="tcl"><?php echo '<a href="'.$feather->url->get('user/'.$info['user_data'][$cur_poster['poster_id']]['id'].'/').'">'.Utils::escape($info['user_data'][$cur_poster['poster_id']]['username']).'</a>' ?></td>
