...
<div class="linkst">
	<div class="inbox crumbsplus">
		<ul class="crumbs">
			...
			<li><span>»&#160;</span><a href="<?php echo $feather->url->get_link('admin/bans/') ?>"><?php _e('Bans') ?></a></li>
        ...
        foreach ($ban_data as $cur_ban) {
            ?>
				<tr>
					...
					<td class="tc6"><?php echo($cur_ban['ban_creator_username'] != '') ? '<a href="'.$feather->url->get_link('user/'.$cur_ban['ban_creator'].'/').'">'.feather_escape($cur_ban['ban_creator_username']).'</a>' : __('Unknown') ?></td>
					<td class="tcr"><?php echo '<a href="'.$feather->url->get_link('admin/bans/edit/'.$cur_ban['id'].'/').'">'.__('Edit').'</a> | <a href="'.$feather->url->get_link('admin/bans/delete/'.$cur_ban['id'].'/').'">'.__('Remove').'</a>' ?></td>
				</tr>
<?php
