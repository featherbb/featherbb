					...
					<fieldset>
						...
						<div class="infldset">
							<table class="aligntop">
								<tr>
									<th scope="row"><?php printf(__('Reported by'), ($report['reporter'] != '') ? '<a href="'.$feather->url->get('users/'.$report['reported_by'].'/').'">'.feather_escape($report['reporter']).'</a>' : __('Deleted user')) ?></th>
...
if (!empty($report_zapped_data)) {
    foreach ($report_zapped_data as $report) {
        ?>
				<div class="inform">
					<fieldset>
						<legend><?php printf(__('Zapped subhead'), format_time($report['zapped']), ($report['zapped_by'] != '') ? '<a href="'.$feather->url->get('user/'.$report['zapped_by_id'].'/').'">'.feather_escape($report['zapped_by']).'</a>' : __('NA')) ?></legend>
						<div class="infldset">
							<table class="aligntop">
								<tr>
									<th scope="row"><?php printf(__('Reported by'), ($report['reporter'] != '') ? '<a href="'.$feather->url->get('users/'.$report['reported_by'].'/').'">'.feather_escape($report['reporter']).'</a>' : __('Deleted user')) ?></th>
