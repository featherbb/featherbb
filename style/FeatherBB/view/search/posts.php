...
<h2><span><span class="conr">#<?php echo($search['start_from'] + $post_count) ?></span> <span><?php if ($cur_search['pid'] != $cur_search['first_post_id']) {
    _e('Re').' ';
} ?><?php echo $forum ?></span> <span>»&#160;<a href="<?php echo $feather->url->get('topic/'.$cur_search['tid'].'/'.$url_topic.'/') ?>"><?php echo feather_escape($cur_search['subject']) ?></a></span> <span>»&#160;<a href="<?php echo $feather->url->get('post/'.$cur_search['pid'].'/#p'.$cur_search['pid']) ?>"><?php echo format_time($cur_search['pposted']) ?></a></span></span></h2>
<div class="box">
	<div class="inbox">
		<div class="postbody">
			<div class="postleft">
	...
	<div class="inbox">
		<div class="postfoot clearb">
			<div class="postfootright">
				<ul>
					<li><span><a href="<?php echo $feather->url->get('topic/'.$cur_search['tid'].'/'.$url_topic.'/') ?>"><?php _e('Go to topic') ?></a></span></li>
					<li><span><a href="<?php echo $feather->url->get('post/'.$cur_search['pid'].'/#p'.$cur_search['pid']) ?>"><?php _e('Go to post') ?></a></span></li>
				</ul>
			</div>
		</div>
	</div>
