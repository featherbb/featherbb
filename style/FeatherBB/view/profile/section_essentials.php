...
<div class="blockform">
	...
	<div class="box">
		<form id="profile1" method="post" action="<?php echo get_link('user/'.$id.'/section/essentials/') ?>" onsubmit="return process_form(this)">
			...
			<div class="inform">
				<fieldset>
					<legend><?php echo __('Username and pass legend') ?></legend>
					<div class="infldset">
						<input type="hidden" name="form_sent" value="1" />
						<?php echo $user_disp['username_field'] ?>
<?php if ($feather->user->id == $id || $feather->user->g_id == FEATHER_ADMIN || ($user['g_moderator'] == '0' && $feather->user->g_mod_change_passwords == '1')): ?>							<p class="actions"><span><a href="<?php echo get_link('user/'.$id.'/action/change_pass/') ?>"><?php echo __('Change pass') ?></a></span></p>
<?php endif; ?>						</div>
				</fieldset>
			</div>
			<div class="inform">
			...
			<div class="inform">
				<fieldset>
					...
					<div class="infldset">
						<p><?php echo __('Time zone info') ?></p>
						<label><?php echo __('Time zone')."\n" ?>
...
} ?> /><?php echo __('DST') ?><br /></label>
						</div>
						<label><?php echo __('Time format') ?>
...
<?php if ($feather->user->is_admmod): ?>							<label><?php echo __('Admin note') ?><br />
