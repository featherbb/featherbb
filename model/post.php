...
class post
{
    ...
    public function get_info_post($tid, $fid)
    {
        ...
        if ($tid) {
            ...
            $cur_posting = \ORM::for_table('topics')
                ->table_alias('t')
            ...
            $cur_posting = \ORM::for_table('forums')
                ->table_alias('f')
                ->select_many($select_get_info_post)
                ->left_outer_join($this->feather->prefix.'forum_perms', array('fp.forum_id', '=', 'f.id'), 'fp')
                ->left_outer_join($this->feather->prefix.'forum_perms', array('fp.group_id', '=', $this->user->g_id), null, true)
    ...
    public function check_errors_before_post($fid, $tid, $qid, $pid, $page, $errors)
    {
        ...
        if ($tid) {
            $subject_tid = \ORM::for_table('topics')
                ->where('id', $tid)
    ...
    public function insert_reply($post, $tid, $cur_posting, $is_subscribed)
    {
        ...
        if (!$this->user->is_guest) {
            ...
            $new['pid'] = \ORM::get_db()->lastInsertId($this->feather->prefix.'posts');
    ...
    public function send_notifications_reply($tid, $cur_posting, $new_pid, $post)
    {
        ...
        $previous_post_time = \ORM::for_table('posts')
            ->where('topic_id', $tid)
            ->order_by_desc('id')
            ->find_one_col('posted');
    ...
    public function insert_topic($post, $fid)
    {
        ...
        $new['tid'] = \ORM::get_db()->lastInsertId($this->feather->prefix.'topics');
        ...
        $new['pid'] = \ORM::get_db()->lastInsertId($this->feather->prefix.'topics');
        ...
        \ORM::for_table('topics')->where('id', $new['tid'])
            ->find_one()
            ->set($update_topic)
            ->save();
    ...
    public function get_quote_message($qid, $tid)
    {
        ...
        $quote = \ORM::for_table('posts')->select_many($select_get_quote_message)
                 ->where('id', $qid)
                 ->where('topic_id', $tid)
                 ->find_one();
    ...
    public function topic_review($tid)
    {
        ...
        $result = \ORM::for_table('posts')->select_many($select_topic_review)
            ->where('topic_id', $tid)
            ->order_by_desc('id')
            ->find_many();
