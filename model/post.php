...
function get_info_post($tid, $fid)
{
    ...
    if ($tid) {
        $result = $db->query('SELECT f.id, f.forum_name, f.moderators, f.redirect_url, fp.post_replies, fp.post_topics, t.subject, t.closed, s.user_id AS is_subscribed FROM '.$db->prefix.'topics AS t INNER JOIN '.$db->prefix.'forums AS f ON f.id=t.forum_id LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$feather_user['g_id'].') LEFT JOIN '.$db->prefix.'topic_subscriptions AS s ON (t.id=s.topic_id AND s.user_id='.$feather_user['id'].') WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND t.id='.$tid) or error('Unable to fetch forum info', __FILE__, __LINE__, $db->error());
    } else {
        $result = $db->query('SELECT f.id, f.forum_name, f.moderators, f.redirect_url, fp.post_replies, fp.post_topics FROM '.$db->prefix.'forums AS f LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$feather_user['g_id'].') WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND f.id='.$fid) or error('Unable to fetch forum info', __FILE__, __LINE__, $db->error());
    }
...
{
    global $db, $feather_user, $feather_config, $lang_post, $lang_common, $lang_prof_reg, $lang_register, $lang_antispam, $lang_antispam_questions, $pd;
    ...
    // Flood protection
    if ($feather->request->post('preview') != '' && $feather_user['last_post'] != '' && (time() - $feather_user['last_post']) < $feather_user['g_post_flood']) {
        $errors[] = sprintf($lang_post['Flood start'], $feather_user['g_post_flood'], $feather_user['g_post_flood'] - (time() - $feather_user['last_post']));
    }
    ...
    if ($fid) {
        ...
        } elseif ($feather_config['o_censoring'] == '1' && $censored_subject == '') {
            $errors[] = $lang_post['No subject after censoring'];
        } elseif (pun_strlen($subject) > 70) {
            $errors[] = $lang_post['Too long subject'];
        } elseif ($feather_config['p_subject_all_caps'] == '0' && is_all_uppercase($subject) && !$feather_user['is_admmod']) {
            $errors[] = $lang_post['All caps subject'];
        }
    }

    // If the user is logged in we get the username and email from $feather_user
    if (!$feather_user['is_guest']) {
        $username = $feather_user['username'];
        $email = $feather_user['email'];
    }
    // Otherwise it should be in $feather ($_POST)
    else {
        $email = strtolower(pun_trim(($feather_config['p_force_guest_email'] == '1') ? $feather->request->post('req_email') : $feather->request->post('email')));
        ...
        if ($feather_config['p_force_guest_email'] == '1' || $email != '') {
            ...
            if ($feather_user['is_guest'] && is_banned_email($email)) {
    ...
    if ($feather_config['p_message_bbcode'] == '1') {
    ...
    if (empty($errors)) {
        ...
        } elseif ($feather_config['o_censoring'] == '1') {
...
{
    global $feather_user, $feather_config;
    
    $post = array();
    
    if (!$feather_user['is_guest']) {
        $post['username'] = $feather_user['username'];
        $post['email'] = $feather_user['email'];
    }
    // Otherwise it should be in $feather ($_POST)
    else {
        $post['username'] = pun_trim($feather->request->post('req_username'));
        $post['email'] = strtolower(pun_trim(($feather_config['p_force_guest_email'] == '1') ? $feather->request->post('req_email') : $feather->request->post('email')));
    }
...
{
    global $db, $feather_user, $feather_config;
    ...
    if (!$feather_user['is_guest']) {
        ...
        // Insert the new post
        $db->query('INSERT INTO '.$db->prefix.'posts (poster, poster_id, poster_ip, message, hide_smilies, posted, topic_id) VALUES(\''.$db->escape($post['username']).'\', '.$feather_user['id'].', \''.$db->escape(get_remote_address()).'\', \''.$db->escape($post['message']).'\', '.$post['hide_smilies'].', '.$post['time'].', '.$tid.')') or error('Unable to create post', __FILE__, __LINE__, $db->error());
        $new['pid'] = $db->insert_id();

        // To subscribe or not to subscribe, that ...
        if ($feather_config['o_topic_subscriptions'] == '1') {
            if (isset($post['subscribe']) && $post['subscribe'] && !$is_subscribed) {
                $db->query('INSERT INTO '.$db->prefix.'topic_subscriptions (user_id, topic_id) VALUES('.$feather_user['id'].' ,'.$tid.')') or error('Unable to add subscription', __FILE__, __LINE__, $db->error());
            } elseif (!isset($post['subscribe']) && $is_subscribed) {
                $db->query('DELETE FROM '.$db->prefix.'topic_subscriptions WHERE user_id='.$feather_user['id'].' AND topic_id='.$tid) or error('Unable to remove subscription', __FILE__, __LINE__, $db->error());
            }
        }
    } else {
        // It's a guest. Insert the new post
        $email_sql = ($feather_config['p_force_guest_email'] == '1' || $post['email'] != '') ? '\''.$db->escape($post['email']).'\'' : 'NULL';
        ...
        while ($cur_subscriber = $db->fetch_assoc($result)) {
            ...
            if (!isset($notification_emails[$cur_subscriber['language']])) {
                if (file_exists(FEATHER_ROOT.'lang/'.$cur_subscriber['language'].'/mail_templates/new_reply.tpl')) {
                    ...
                    $mail_message_full = str_replace('<board_mailer>', $feather_config['o_board_title'], $mail_message_full);
...
function insert_topic($post, $fid)
{
    ...
    if (!$feather_user['is_guest']) {
        // To subscribe or not to subscribe, that ...
        if ($feather_config['o_topic_subscriptions'] == '1' && $post['subscribe']) {
            $db->query('INSERT INTO '.$db->prefix.'topic_subscriptions (user_id, topic_id) VALUES('.$feather_user['id'].' ,'.$new['tid'].')') or error('Unable to add subscription', __FILE__, __LINE__, $db->error());
        }

        // Create the post ("topic post")
        $db->query('INSERT INTO '.$db->prefix.'posts (poster, poster_id, poster_ip, message, hide_smilies, posted, topic_id) VALUES(\''.$db->escape($post['username']).'\', '.$feather_user['id'].', \''.$db->escape(get_remote_address()).'\', \''.$db->escape($post['message']).'\', '.$post['hide_smilies'].', '.$post['time'].', '.$new['tid'].')') or error('Unable to create post', __FILE__, __LINE__, $db->error());
    } else {
        // Create the post ("topic post")
        $email_sql = ($feather_config['p_force_guest_email'] == '1' || $post['email'] != '') ? '\''.$db->escape($post['email']).'\'' : 'NULL';
...
function send_notifications_new_topic($post, $cur_posting, $new_tid)
{
    ...
    // Get any subscribed users that should be notified (banned users are excluded)
    $result = $db->query('SELECT u.id, u.email, u.notify_with_post, u.language FROM '.$db->prefix.'users AS u INNER JOIN '.$db->prefix.'forum_subscriptions AS s ON u.id=s.user_id LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id='.$cur_posting['id'].' AND fp.group_id=u.group_id) LEFT JOIN '.$db->prefix.'bans AS b ON u.username=b.username WHERE b.username IS NULL AND (fp.read_forum IS NULL OR fp.read_forum=1) AND s.forum_id='.$cur_posting['id'].' AND u.id!='.$feather_user['id']) or error('Unable to fetch subscription info', __FILE__, __LINE__, $db->error());
    if ($db->num_rows($result)) {
        require_once FEATHER_ROOT.'include/email.php';
        ...
        while ($cur_subscriber = $db->fetch_assoc($result)) {
            ...
            if (!isset($notification_emails[$cur_subscriber['language']])) {
                if (file_exists(FEATHER_ROOT.'lang/'.$cur_subscriber['language'].'/mail_templates/new_topic.tpl')) {
                    ...
                    $mail_message = str_replace('<topic_subject>', $feather_config['o_censoring'] == '1' ? $censored_subject : $post['subject'], $mail_message);
                    ...
                    $mail_message_full = str_replace('<topic_subject>', $feather_config['o_censoring'] == '1' ? $censored_subject : $post['subject'], $mail_message_full);
...
function warn_banned_user($post, $new_pid)
{
    ...
    $mail_tpl = trim(file_get_contents(FEATHER_ROOT.'lang/'.$feather_user['language'].'/mail_templates/banned_email_post.tpl'));
...
function increment_post_count($post, $new_tid)
{
    ...
    if (!$feather_user['is_guest']) {
        $db->query('UPDATE '.$db->prefix.'users SET num_posts=num_posts+1, last_post='.$post['time'].' WHERE id='.$feather_user['id']) or error('Unable to update user', __FILE__, __LINE__, $db->error());

        // Promote this user to a new group if enabled
        if ($feather_user['g_promote_next_group'] != 0 && $feather_user['num_posts'] + 1 >= $feather_user['g_promote_min_posts']) {
            $new_group_id = $feather_user['g_promote_next_group'];
            $db->query('UPDATE '.$db->prefix.'users SET group_id='.$new_group_id.' WHERE id='.$feather_user['id']) or error('Unable to promote user to new group', __FILE__, __LINE__, $db->error());
        }
...
function get_quote_message($qid, $tid)
{
    ...
    if ($feather_config['p_message_bbcode'] == '1') {    // Sanitize username for inclusion within QUOTE BBCode attribute.
...
{
    global $feather_user, $lang_post, $lang_common, $feather_config;
    ...
    if (!$feather_user['is_guest']) {
        ...
        if ($feather_config['o_topic_subscriptions'] == '1') {
            ...
            // If auto subscribed
            elseif ($feather_user['auto_notify']) {
                $subscr_checked = true;
            }
    ...
    } elseif ($feather_config['o_smilies'] == '1') {
        $checkboxes[] = '<label><input type="checkbox" name="hide_smilies" value="1" tabindex="'.($cur_index++).'"'.($feather->request->post('hide_smilies') ? ' checked="checked"' : '').' />'.$lang_post['Hide smilies'].'<br /></label>';
    }
...
{
    global $db, $feather_config, $pd;
    
    $post_data = array();

    require_once FEATHER_ROOT.'include/parser.php';

    $result = $db->query('SELECT poster, message, hide_smilies, posted FROM '.$db->prefix.'posts WHERE topic_id='.$tid.' ORDER BY id DESC LIMIT '.$feather_config['o_topic_review']) or error('Unable to fetch topic review', __FILE__, __LINE__, $db->error());
