...
class post
{
        ...
        if ($result) {
            ...
            foreach($result as $cur_subscriber) {
                ...
                if (!isset($notification_emails[$cur_subscriber['language']])) {
                    if (file_exists(FEATHER_ROOT.'app/lang/'.$cur_subscriber['language'].'/mail_templates/new_reply.tpl')) {
                        // Load the "new reply" template
                        $mail_tpl = trim(file_get_contents(FEATHER_ROOT.'app/lang/'.$cur_subscriber['language'].'/mail_templates/new_reply.tpl'));
                        $mail_tpl = $this->hook->fire('send_notifications_reply_mail_tpl', $mail_tpl);

                        // Load the "new reply full" template (with post included)
                        $mail_tpl_full = trim(file_get_contents(FEATHER_ROOT.'app/lang/'.$cur_subscriber['language'].'/mail_templates/new_reply_full.tpl'));
    ...
    public function send_notifications_new_topic($post, $cur_posting, $new_tid)
    {
        ...
        if ($result) {
            ...
            foreach($result as $cur_subscriber) {
                // Is the subscription email for $cur_subscriber['language'] cached or not?
                if (!isset($notification_emails[$cur_subscriber['language']])) {
                    if (file_exists(FEATHER_ROOT.'app/lang/'.$cur_subscriber['language'].'/mail_templates/new_topic.tpl')) {
                        // Load the "new topic" template
                        $mail_tpl = trim(file_get_contents(FEATHER_ROOT.'app/lang/'.$cur_subscriber['language'].'/mail_templates/new_topic.tpl'));
                        ...
                        $mail_tpl_full = trim(file_get_contents(FEATHER_ROOT.'app/lang/'.$cur_subscriber['language'].'/mail_templates/new_topic_full.tpl'));
    ...
    public function warn_banned_user($post, $new_pid)
    {
        ...
        $mail_tpl = trim(file_get_contents(FEATHER_ROOT.'app/lang/'.$this->user->language.'/mail_templates/banned_email_post.tpl'));
