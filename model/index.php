...
class index
{
    ...
    public function print_categories_forums()
    {
        ...
        foreach ($query as $cur_forum) {
            ...
            if ($cur_forum->redirect_url != '') {
                ...
                $cur_forum->forum_field = '<h3><a href="'.$this->feather->url->get('forum/'.$cur_forum->fid.'/'.$this->feather->url->url_friendly($cur_forum->forum_name)).'/'.'">'.feather_escape($cur_forum->forum_name).'</a>'.(!empty($forum_field_new) ? ' '.$forum_field_new : '').'</h3>';
            ...
            if ($cur_forum->last_post != '') {
                $cur_forum->last_post_formatted = '<a href="'.$this->feather->url->get('post/'.$cur_forum->last_post_id.'/#p'.$cur_forum->last_post_id).'">'.format_time($cur_forum->last_post).'</a> <span class="byuser">'.__('by').' '.feather_escape($cur_forum->last_poster).'</span>';
            ...
            if ($cur_forum->moderators != '') {
                ...
                foreach ($mods_array as $mod_username => $mod_id) {
                    if ($this->user->g_view_users == '1') {
                        $moderators[] = '<a href="'.$this->feather->url->get('user/'.$mod_id.'/').'">'.feather_escape($mod_username).'</a>';
                    } else {
    ...
    public function collect_stats()
    {
        ...
        if ($this->user->g_view_users == '1') {
            $stats['newest_user'] = '<a href="'.$this->feather->url->get('user/'.$stats['last_user']['id']).'/">'.feather_escape($stats['last_user']['username']).'</a>';
        } else {
    ...
    public function fetch_users_online()
    {
        ...
        foreach($query as $user_online) {
            if ($user_online->user_id > 1) {
                if ($this->user->g_view_users == '1') {
                    $online['users'][] = "\n\t\t\t\t".'<dd><a href="'.$this->feather->url->get('user/'.$user_online->user_id).'/">'.feather_escape($user_online->ident).'</a>';
                } else {
