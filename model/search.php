...
class search
{
    ...
    public function get_search_results()
    {
        ...
        if (!$this->user->is_guest && $search_type[0] == 'action' && $search_type[1] == 'show_new') {
            $search['forum_actions'][] = '<a href="'.$this->feather->url->get_link('mark-read/').'">'.__('Mark all as read').'</a>';
        }
        ...
        if (!empty($search_ids)) {
            ...
            if ($search_type[0] == 'action') {
                if ($search_type[1] == 'show_user_topics') {
                    $search['crumbs_text']['search_type'] = '<a href="'.$this->feather->url->get_link('search/?action=show_user_topics&amp;user_id='.$search_type[2]).'">'.sprintf(__('Quick search show_user_topics'), feather_escape($search['search_set'][0]['poster'])).'</a>';
                } elseif ($search_type[1] == 'show_user_posts') {
                    $search['crumbs_text']['search_type'] = '<a href="'.$this->feather->url->get_link('search/?action=show_user_posts&amp;user_id='.$search_type[2]).'">'.sprintf(__('Quick search show_user_posts'), feather_escape($search['search_set'][0]['pposter'])).'</a>';
                    ...
                    $search['crumbs_text']['search_type'] = '<a href="'.$this->feather->url->get_link('search/?action=show_subscription&amp;user_id='.$subscriber_id).'">'.sprintf(__('Quick search show_subscriptions'), feather_escape($subscriber_name)).'</a>';
                } else {
                    $search_url = str_replace('_', '/', $search_type[1]);
                    $search['crumbs_text']['search_type'] = '<a href="'.$this->feather->url->get_link('search/'.$search_url.'/').'">'.__('Quick search '.$search_type[1]).'</a>';
                }
            } else {
                $keywords = $author = '';
                ...
                $search['crumbs_text']['search_type'] = '<a href="'.$this->feather->url->get_link('search/?action=search&amp;keywords='.urlencode($keywords).'&amp;author='.urlencode($author).'&amp;forums='.$search_type[2].'&amp;search_in='.$search_type[3].'&amp;sort_by='.$sort_by.'&amp;sort_dir='.$sort_dir.'&amp;show_as='.$show_as).'">'.$search['crumbs_text']['search_type'].'</a>';
            }
        }
    ...
    public function display_search_results($search)
    {
        ...
        foreach ($search['search_set'] as $cur_search) {
            $forum = '<a href="'.$this->feather->url->get_link('forum/'.$cur_search['forum_id'].'/'.$this->feather->url->url_friendly($cur_search['forum_name']).'/').'">'.feather_escape($cur_search['forum_name']).'</a>';
            ...
            if ($search['show_as'] == 'posts') {
                ...
                if ($cur_search['poster_id'] > 1 && $this->user->g_view_users == '1') {
                    $cur_search['pposter_disp'] = '<strong><a href="'.$this->feather->url->get_link('user/'.$cur_search['poster_id'].'/').'">'.$pposter.'</a></strong>';
                } else {
                ...
                $subject = '<a href="'.$this->feather->url->get_link('topic/'.$cur_search['tid'].'/'.$url_topic.'/').'">'.feather_escape($cur_search['subject']).'</a> <span class="byuser">'.__('by').' '.feather_escape($cur_search['poster']).'</span>';
                ...
                if (!$this->user->is_guest && $cur_search['last_post'] > $this->user->last_visit && (!isset($tracked_topics['topics'][$cur_search['tid']]) || $tracked_topics['topics'][$cur_search['tid']] < $cur_search['last_post']) && (!isset($tracked_topics['forums'][$cur_search['forum_id']]) || $tracked_topics['forums'][$cur_search['forum_id']] < $cur_search['last_post'])) {
                    ...
                    $subject_new_posts = '<span class="newtext">[ <a href="'.$this->feather->url->get_link('topic/'.$cur_search['tid'].'/action/new/').'" title="'.__('New posts info').'">'.__('New posts').'</a> ]</span>';
                } else {
