...
class maintenance
{
    ...
    public function rebuild()
    {
        ...
        if ($this->request->get('i_empty_index')) {
            \ORM::for_table($this->db->prefix.'search_words')->raw_execute('TRUNCATE '.$this->db->prefix.'search_words');
            \ORM::for_table($this->db->prefix.'search_matches')->raw_execute('TRUNCATE '.$this->db->prefix.'search_matches');
            ...
            switch ($db_type) {
                ...
                case 'mysql_innodb':
                case 'mysqli_innodb':
                    \ORM::for_table($this->db->prefix.'search_words')->raw_execute('ALTER TABLE '.$this->db->prefix.'search_words auto_increment=1');
                    break;

                case 'pgsql';
                    \ORM::for_table($this->db->prefix.'search_words')->raw_execute('SELECT setval(\''.$this->db->prefix.'search_words_id_seq\', 1, false)');
            }
        }
    }
    ...
    public function get_query_str()
    {
        ...
        $result = \ORM::for_table($this->db->prefix.'posts')->table_alias('p')
    ...
    public function prune($forum_id, $prune_sticky, $prune_date)
    {
        ...
        $topics_id = \ORM::for_table($this->db->prefix.'topics')->select('id')
        ...
        if (!empty($topic_ids)) {
            ...
            $posts_id = \ORM::for_table($this->db->prefix.'posts')->select('id')
    ...
    public function prune_comply($prune_from, $prune_sticky)
    {
        ...
        if ($prune_from == 'all') {
            $result = \ORM::for_table($this->db->prefix.'forums')->select('id')->find_array();
        ...
        $result = \ORM::for_table($this->db->prefix.'topics')->table_alias('t1')
                        ->select('t1.id')
                        ->left_outer_join($this->db->prefix.'topics', array('t1.moved_to', '=', 't2.id'), 't2')
                        ->where_null('t2.id')
    ...
    public function get_info_prune($prune_sticky, $prune_from)
    {
        ...
        $query = \ORM::for_table($this->db->prefix.'topics')->where_lt('last_post', $prune['date'])
                        ->where_null('moved_to');
        ...
        if ($prune_from != 'all') {
            ...
            $forum = \ORM::for_table($this->db->prefix.'forums')->where('id', $prune_from)
    ...
    {
        $first_id = '';
        $first_id_sql = \ORM::for_table($this->feather->prefix.'posts')->order_by_asc('id')
                            ->find_one_col('id');
        if ($first_id_sql) {
