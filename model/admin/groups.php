...
function info_add_group($groups, $feather, $id)
{
    ...
    if (!empty($feather->request->post('add_group'))) {
...
function add_edit_group($groups, $feather)
{
    ...
    if(!empty($feather->request->post('group_id'))) {
	...
	$is_admin_group = (!empty($feather->request->post('group_id')) && $feather->request->post('group_id') == PUN_ADMIN) ? true : false;

	$title = pun_trim($feather->request->post('req_title'));
	$user_title = pun_trim($feather->request->post('user_title'));

    $promote_min_posts = !empty($feather->request->post('promote_min_posts')) ? intval($feather->request->post('promote_min_posts')) : '0';
	if (!empty($feather->request->post('promote_next_group')) &&
			...
			(!empty($feather->request->post('group_id')) || $feather->request->post('promote_next_group') != $feather->request->post('group_id'))) {
	...
	$moderator = !empty($feather->request->post('moderator')) && $feather->request->post('moderator') == '1' ? '1' : '0';
	$mod_edit_users = $moderator == '1' && !empty($feather->request->post('mod_edit_users')) && $feather->request->post('mod_edit_users') == '1' ? '1' : '0';
	$mod_rename_users = $moderator == '1' && !empty($feather->request->post('mod_rename_users')) && $feather->request->post('mod_rename_users') == '1' ? '1' : '0';
	$mod_change_passwords = $moderator == '1' && !empty($feather->request->post('mod_change_passwords')) && $feather->request->post('mod_change_passwords') == '1' ? '1' : '0';
	$mod_ban_users = $moderator == '1' && !empty($feather->request->post('mod_ban_users')) && $feather->request->post('mod_ban_users') == '1' ? '1' : '0';
	$mod_promote_users = $moderator == '1' && !empty($feather->request->post('mod_promote_users')) && $feather->request->post('mod_promote_users') == '1' ? '1' : '0';
	$read_board = !empty($feather->request->post('read_board')) ? $feather->request->post('read_board') : '1';
	$view_users = (!empty($feather->request->post('view_users')) && $feather->request->post('view_users') == '1') || $is_admin_group ? '1' : '0';
	$post_replies = !empty($feather->request->post('post_replies')) ? $feather->request->post('post_replies') : '1';
	$post_topics = !empty($feather->request->post('post_topics')) ? $feather->request->post('post_topics') : '1';
	$edit_posts = !empty($feather->request->post('edit_posts')) ? $feather->request->post('edit_posts') : ($is_admin_group) ? '1' : '0';
	$delete_posts = !empty($feather->request->post('delete_posts')) ? $feather->request->post('delete_posts') : ($is_admin_group) ? '1' : '0';
	$delete_topics = !empty($feather->request->post('delete_topics')) ? $feather->request->post('delete_topics') : ($is_admin_group) ? '1' : '0';
	$post_links = !empty($feather->request->post('post_links')) ? $feather->request->post('post_links') : '1';
	$set_title = !empty($feather->request->post('set_title')) ? $feather->request->post('set_title') : ($is_admin_group) ? '1' : '0';
	$search = !empty($feather->request->post('search')) ? $feather->request->post('search') : '1';
	$search_users = !empty($feather->request->post('search_users')) ? $feather->request->post('search_users') : '1';
	$send_email = (!empty($feather->request->post('send_email')) && $feather->request->post('send_email') == '1') || $is_admin_group ? '1' : '0';
	$post_flood = (!empty($feather->request->post('post_flood')) && $feather->request->post('post_flood') >= 0) ? $feather->request->post('post_flood') : '0';
	$search_flood = (!empty($feather->request->post('search_flood')) && $feather->request->post('search_flood') >= 0) ? $feather->request->post('search_flood') : '0';
	$email_flood = (!empty($feather->request->post('email_flood')) && $feather->request->post('email_flood') >= 0) ? $feather->request->post('email_flood') : '0';
	$report_flood = (!empty($feather->request->post('report_flood')) && $feather->request->post('report_flood') >= 0) ? $feather->request->post('report_flood') : '0';
...
function delete_group($feather, $group_id)
{
	...
	if (!empty($feather->request->post('del_group'))) {
