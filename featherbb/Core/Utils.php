...
class Utils
{
    ...
    public static function get_title($user)
    {
        ...
        elseif ($user['g_id'] == Container::get('forum_env')['FEATHER_GUEST']) {
            $user_title = __('Guest');
        }
    ...
    public static function generate_avatar_markup($user_id)
    {
        ...
        foreach ($filetypes as $cur_type) {
            ...
            if (file_exists(Container::get('forum_env')['FEATHER_ROOT'].$path) && $img_size = getimagesize(Container::get('forum_env')['FEATHER_ROOT'].$path)) {
                //$avatar_markup = '<img src="'.\FeatherBB\Core\Utils::escape($feather->url->base(true).'/'.$path.'?m='.filemtime(Container::get('forum_env')['FEATHER_ROOT'].$path)).'" '.$img_size[3].' alt="" />'; TODO
                break;
            }
        }
