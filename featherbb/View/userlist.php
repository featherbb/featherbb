            ...
            foreach ($userlist_data as $user) {
                ?>
                    <tr>
                        ...
                        <td class="tc2"><?= Utils::getTitle($user) ?></td>
    <?php if ($show_post_count): ?>                    <td class="tc3"><?= Utils::forumNumberFormat($user['num_posts']) ?></td>
    <?php endif; ?>
                        <td class="tcr"><?= Utils::formatTime($user['registered'], true) ?></td>
                    </tr>
            <?php
