...
<?php if (Container::get('user')->id == $id || Container::get('user')->g_id == Config::get('forum_env')['FEATHER_ADMIN'] || ($user['g_moderator'] == '0' && Container::get('user')->g_mod_change_passwords == '1')): ?>                            <p class="actions"><span><a href="<?= Router::pathFor('profileAction', ['id' => $id, 'action' => 'change_pass']) ?>"><?php _e('Change pass') ?></a></span></p>
<?php endif; ?>                        </div>
                </fieldset>
            </div>
            <div class="inform">
