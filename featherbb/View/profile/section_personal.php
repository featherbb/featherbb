...
<?php if (Container::get('user')->g_post_links == '1' || Container::get('user')->g_id == Config::get('forum_env')['FEATHER_ADMIN']) : ?>                            <label><?php _e('Website') ?><br /><input type="text" name="form_url" value="<?= Utils::escape($user['url']) ?>" size="50" maxlength="80" /><br /></label>
<?php endif; ?>
                    </div>
                </fieldset>
            </div>
