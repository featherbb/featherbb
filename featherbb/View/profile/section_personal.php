...
<?php if (isset($title_field)): ?>
                                        <?= $title_field ?>
<?php endif; ?>
                                        <label><?= __('Location') ?><br /><input type="text" name="form_location" value="<?= Utils::escape($user['location']) ?>" size="30" maxlength="30" /><br /></label>
<?php if (User::can('post.links') || User::isAdmin()) : ?>
                                        <label><?= __('Website') ?><br /><input type="text" name="form_url" value="<?= Utils::escape($user['url']) ?>" size="50" maxlength="80" /><br /></label>
<?php endif; ?>
                                        <label><?= __('API token') ?><br /><input type="text" name="api" readonly="readonly" value="<?= Api::getToken(User::get()) ?>" size="60" maxlength="60" /><br /></label>
                                    </div>
                                </fieldset>
                            </div>
