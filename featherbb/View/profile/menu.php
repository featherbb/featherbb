...
<div id="profile" class="block2col">
    ...
    ?>><a href="<?php echo $feather->urlFor('profileSection', ['id' => $id, 'section' => 'essentials']) ?>"><?php _e('Section essentials') ?></a></li>
					<li<?php if ($page == 'personal') {
    echo ' class="isactive"';
}
    ?>><a href="<?php echo $feather->urlFor('profileSection', ['id' => $id, 'section' => 'personal']) ?>"><?php _e('Section personal') ?></a></li>
					<li<?php if ($page == 'messaging') {
    echo ' class="isactive"';
}
    ?>><a href="<?php echo $feather->urlFor('profileSection', ['id' => $id, 'section' => 'messaging']) ?>"><?php _e('Section messaging') ?></a></li>
<?php if ($feather->forum_settings['o_avatars'] == '1' || $feather->forum_settings['o_signatures'] == '1'): ?>					<li<?php if ($page == 'personality') {
    echo ' class="isactive"';
}
    ?>><a href="<?php echo $feather->urlFor('profileSection', ['id' => $id, 'section' => 'personality']) ?>"><?php _e('Section personality') ?></a></li>
<?php endif;
    ?>					<li<?php if ($page == 'display') {
    echo ' class="isactive"';
}
    ?>><a href="<?php echo $feather->urlFor('profileSection', ['id' => $id, 'section' => 'display']) ?>"><?php _e('Section display') ?></a></li>
