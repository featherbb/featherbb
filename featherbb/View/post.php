...
if ($fid): ?>
                        <label class="required"><strong><?= __('Subject') ?> <span><?= __('Required') ?></span></strong><br /><input class="longinput" type="text" name="req_subject" value="<?= Input::post('req_subject') ?>" size="80" maxlength="70" tabindex="<?= $cur_index++ ?>" required="required"<?= (!User::get()->is_guest) ? ' autofocus' : '' ?> /><br /></label>
<?php endif; ?>                        <label class="required"><strong><?= __('Message') ?> <span><?= __('Required') ?></span></strong><br />
                        ...
                        <ul class="bblinks">
                            ...
                            <li><span><a href="<?= Router::pathFor('help').'#smilies' ?>" onclick="window.open(this.href); return false;"><?= __('Smilies') ?></a> <?= (ForumSettings::get('smilies') == '1') ? __('on') : __('off'); ?></span></li>
                        </ul>
                    </div>
                </fieldset>
<?php
