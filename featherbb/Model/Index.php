...
class Index
{
    ...
    public function get_forum_actions()
    {
        ...
        if (!$this->user->is_guest) {
            $forum_actions[] = '<a href="'.$this->feather->url->get('mark-read/').'">'.__('Mark all as read').'</a>';
        }
    ...
    public function print_categories_forums()
    {
        ...
        foreach ($query as $cur_forum) {
            ...
            if (isset($new_topics[$cur_forum->fid])) {
                ...
                $forum_field_new = '<span class="newtext">[ <a href="'.$this->feather->url->get('search/?action=show_new&amp;fid='.$cur_forum->fid).'">'.__('New posts').'</a> ]</span>';
                $cur_forum->icon_type = 'icon icon-new';
            }

            // Is this a redirect forum?
            if ($cur_forum->redirect_url != '') {
                $cur_forum->forum_field = '<h3><span class="redirtext">'.__('Link to').'</span> <a href="'.$this->feather->utils->escape($cur_forum->redirect_url).'" title="'.__('Link to').' '.$this->feather->utils->escape($cur_forum->redirect_url).'">'.$this->feather->utils->escape($cur_forum->forum_name).'</a></h3>';
                $cur_forum->num_topics_formatted = $cur_forum->num_posts_formatted = '-';
                $cur_forum->item_status .= ' iredirect';
                $cur_forum->icon_type = 'icon';
            } else {
                $cur_forum->forum_field = '<h3><a href="'.$this->feather->url->get('forum/'.$cur_forum->fid.'/'.$this->feather->url->url_friendly($cur_forum->forum_name)).'/'.'">'.$this->feather->utils->escape($cur_forum->forum_name).'</a>'.(!empty($forum_field_new) ? ' '.$forum_field_new : '').'</h3>';
            ...
            if ($cur_forum->last_post != '') {
                $cur_forum->last_post_formatted = '<a href="'.$this->feather->url->get('post/'.$cur_forum->last_post_id.'/#p'.$cur_forum->last_post_id).'">'.$this->feather->utils->format_time($cur_forum->last_post).'</a> <span class="byuser">'.__('by').' '.$this->feather->utils->escape($cur_forum->last_poster).'</span>';
            ...
            if ($cur_forum->moderators != '') {
                ...
                foreach ($mods_array as $mod_username => $mod_id) {
                    if ($this->user->g_view_users == '1') {
                        $moderators[] = '<a href="'.$this->feather->url->get('user/'.$mod_id.'/').'">'.$this->feather->utils->escape($mod_username).'</a>';
                    } else {
                        $moderators[] = $this->feather->utils->escape($mod_username);
                    }
                }
    ...
    public function collect_stats()
    {
        ...
        if ($this->user->g_view_users == '1') {
            $stats['newest_user'] = '<a href="'.$this->feather->url->get('user/'.$stats['last_user']['id']).'/">'.$this->feather->utils->escape($stats['last_user']['username']).'</a>';
        } else {
            $stats['newest_user'] = $this->feather->utils->escape($stats['last_user']['username']);
        }
    ...
    public function fetch_users_online()
    {
        ...
        foreach($query as $user_online) {
            if ($user_online->user_id > 1) {
                if ($this->user->g_view_users == '1') {
                    $online['users'][] = "\n\t\t\t\t".'<dd><a href="'.$this->feather->url->get('user/'.$user_online->user_id).'/">'.$this->feather->utils->escape($user_online->ident).'</a>';
                } else {
                    $online['users'][] = "\n\t\t\t\t".'<dd>'.$this->feather->utils->escape($user_online->ident);
                }
            } else {
                ++$online['num_guests'];
            }
