...
function generate_config_cache()
{
    ...
    $result = \ORM::for_table($db->prefix.'config')->find_array();
...
function generate_bans_cache()
{
    ...
    $result = \ORM::for_table($db->prefix.'bans')->find_array();
...
function generate_quickjump_cache($group_id = false)
{
    ...
    if ($group_id !== false) {
        ...
        $read_board = \ORM::for_table($db->prefix.'groups')->where('g_id', $group_id)
        ...
        $result = \ORM::for_table($db->prefix.'groups')->select_many($select_quickjump_all_groups)
                        ->find_many();
    ...
    foreach ($groups as $group_id => $read_board) {
        ...
        if ($read_board == '1') {
            ...
            $result = \ORM::for_table($db->prefix.'categories')
                            ->table_alias('c')
                            ->select_many($select_generate_quickjump_cache)
                            ->inner_join($db->prefix.'forums', array('c.id', '=', 'f.cat_id'), 'f')
                            ->left_outer_join($db->prefix.'forum_perms', array('fp.forum_id', '=', 'f.id'), 'fp')
                            ->left_outer_join($db->prefix.'forum_perms', array('fp.group_id', '=', $group_id), null, true)
...
function generate_censoring_cache()
{
    ...
    $result = \ORM::for_table($db->prefix.'censoring')->select_many($select_generate_censoring_cache)
                    ->find_many();
...
function generate_users_info_cache()
{
    ...
    $stats['total_users'] = (\ORM::for_table($db->prefix.'users')->where_not_equal('group_id', FEATHER_UNVERIFIED)
                                ->count('id')) - 1;

    $select_generate_users_info_cache = array('id', 'username');
    $last_user = \ORM::for_table($db->prefix.'users')->select_many($select_generate_users_info_cache)
...
function generate_admins_cache()
{
    ...
    $result = \ORM::for_table($db->prefix.'users')->select('id')
