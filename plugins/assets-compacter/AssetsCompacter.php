    ...
    public function compactAssets()
    {
        ...
        foreach (Input::post('themes') as $theme => $assets) {
            ...
            foreach ($assets['scripts'] as $script) {
                $minifiedJs .= JShrink::minify(file_get_contents(ForumEnv::get('FEATHER_ROOT').$script));
            }

            // Generate stylesheets header
            $minifiedCssHeader = "/*********************************************************************\n";
            $minifiedCssHeader .= " *    Assets Compacter plugin for FeatherBB - minified stylesheets\n";
            $minifiedCssHeader .= " *    Generated : ".date("Y-m-d H:i:s")."\n";
            $minifiedCssHeader .= " *    Content :\n *      - ".join($assets['stylesheets'], "\n *      - ")."\n";
            $minifiedCssHeader .= " **********************************************************************/\n\n";
            // Generate scripts header
            $minifiedJsHeader = "/*********************************************************************\n";
            $minifiedJsHeader .= " *    Assets Compacter plugin for FeatherBB - minified scripts\n";
            $minifiedJsHeader .= " *    Generated : ".date("Y-m-d H:i:s")."\n";
            $minifiedJsHeader .= " *    Content :\n *      - ".join($assets['scripts'], "\n *      - ")."\n";
            $minifiedJsHeader .= " **********************************************************************/\n\n";

            // Write minified assets in destination folder
            file_put_contents($themeFolder.DIRECTORY_SEPARATOR.self::$_compactedStyles, $minifiedCssHeader.$minifiedCss);
            file_put_contents($themeFolder.DIRECTORY_SEPARATOR.self::$_compactedScripts, $minifiedJsHeader.$minifiedJs);
        }

        return array('success', __('Minification success'));
    }
