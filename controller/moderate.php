    ...
    class Moderate{
        ...
        function moderatetopic($id = null, $fid = null, $action = null, $param = null){
			...
			if ($action == 'moderate') {
				...
				if (!empty($feather->request->post('delete_posts')) || !empty($feather->request->post('delete_posts_comply'))) {
					$posts = delete_posts($feather, $id, $fid);
				...
				if (!empty($feather->request->post('split_posts')) || !empty($feather->request->post('split_posts_comply'))) {
					$posts = split_posts($feather, $id, $fid);
		...
		elseif (!empty($feather->request->post('delete_topics')) || !empty($feather->request->post('delete_topics_comply'))) {
			$topics = !empty($feather->request->post('topics')) ? $feather->request->post('topics') : array();
			if (empty($topics)) {
				message($lang_misc['No topics selected']);
			}
