...
class categories
{
    ...
    public function add_category()
    {
        ...
        if ($cat_name == '') {
            redirect($this->feather->url->get_link('admin/categories/'), __('Must enter name message'));
        }

        if ($this->model->add_category($cat_name)) {
            redirect($this->feather->url->get_link('admin/categories/'), __('Category added redirect'));
    ...
    public function edit_categories()
    {
        ...
        redirect($this->feather->url->get_link('admin/categories/'), __('Categories updated redirect'));
    }
    ...
    public function delete_category()
    {
        ...
        if (intval($this->request->post('disclaimer')) != 1) {
            redirect($this->feather->url->get_link('admin/categories/'), __('Delete category not validated'));
        }

        if ($this->model->delete_category($cat_to_delete)) {
            redirect($this->feather->url->get_link('admin/categories/'), __('Category deleted redirect'));
        } else {
            redirect($this->feather->url->get_link('admin/categories/'), __('Unable to delete category'));
        }
    }
